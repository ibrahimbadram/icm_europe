...
class MY_Image_lib extends CI_Image_lib
{
    ...
    }		public function image_compress($uploaddir, $file_name, $quality=60){				//Compress Images process		require_once('Compress.php');  // load the codeginiter library		$file 			= base_url().'uploads/'.$uploaddir.'/'.$file_name; // file that you wanna compress		$new_name_image = $file_name; // name of new file compressed		$destination 	= base_url().'uploads/'.$uploaddir.'/'; // This destination must be exist on your project				//get and instance from compress class		$compress 					= new Compress();		$compress->file_url 		= $file;		$compress->new_name_image 	= $new_name_image;		$compress->quality 			= $quality;		$compress->destination 		= $destination;				$result = $compress->compress_image();			}
