...
class Pages extends BaseController
{
    ...
    function addNewrow()
    {
        ...
        else
        {
            ...
            else
            {
				...
				$url_title = str_replace(array(" ","&",",","(",")",".","!","?",'"',"+","*","/","'"),array("-"),$this->input->post("title_en"));
                ...
                redirect(site_url('admin/pages/addNew'));
            }
        }
    }
    ...
    function editrow()
    {
            ...
            else
            {
                ...
                if(!empty($data_languages))
                    {
                        foreach($data_languages as $record)
                        {
							if ( $record->prefix =='en' && $this->session->userdata ( 'translate_role' ) != ROLE_ADMIN ) {} else {
								if ( $this->input->post('save_all') == 'Save '.$record->title ) {
						$Info['title_'.$record->prefix]=$this->input->post('title_'.$record->prefix);
						$Info['description_'.$record->prefix]=$this->input->post('description_'.$record->prefix);						
                ...
                redirect(site_url('admin/pages/editOld/'.$Id));
            }
        }
