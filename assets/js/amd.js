...
if (  $(window).width() > 1035 ) {
	...
	//hide sub menu on window scroll
	$window.scroll(hide_sub_menu);
}
},function() {
	hide_sub_menu();
});
...
function slide_inner_menu(id,url) {
	...
	if (  $(window).width() < 1035 ) {
		if ( $('.inner_menu_'+id).hasClass('active') ) {
			...
			$('#parent_'+id).removeClass('parent_menu_active');
		} else {
			$('.menu_inner').removeClass('active_inner');
			$('.parent_menu').removeClass('parent_menu_active');
			$('.inner_menu_'+id).addClass('active');
			$('.inner_menu_'+id).addClass('active_inner');
			$('#parent_'+id).addClass('parent_menu_active');
		}
	} else {
...
function showpopup($containerId){
	
	if($containerId != ""){
		$($containerId+'.popup-section,.popup-shadow').fadeIn('fast');
	}else{
...
function hidepopup($containerId){
	...
	if($containerId != ""){
		$('.popup-section-2,'+$containerId+'.popup-section,.popup-shadow').fadeOut('fast');	
	}else{
