...
class Icm_access extends CI_Controller {
     public function __construct()    {        parent::__construct();		$this->load->view('widgets/common');		$this->lg = $this->lang->lang();    }  
    public function index($title='') {
        ...
        $this->template->title = $this->sm->getword('ICM',$this->lg).' - '.$this->sm->getword('ICM ACCESS',$this->lg);
        $this->template->description = $this->sm->getword('ICM ACCESS',$this->lg);
		$this->template->trigger_meta('keyword',$this->sm->getword('ICM ACCESS',$this->lg));
        ...
        $data = array();
		$breadcrumb['breadcrumb'] = array(0=>array('link'=>'home','title'=>$this->sm->getword('home',$this->lg)),1=>array('link'=>'account','title'=>$this->sm->getword('Accounts',$this->lg)),2=>array('link'=>'#','title'=>$this->sm->getword('ICM ACCESS',$this->lg)));
		$this->template->content->view('breadcrumb', $breadcrumb);
        $this->template->content->view('icm-access', $data);
