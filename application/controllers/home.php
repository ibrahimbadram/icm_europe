...
class Home extends CI_Controller {
    ...
    public function index() {
		...
		$this->template->javascript->add('assets/js/jquery.gradientify.min.js');
	...
	public function send_newsletter(){
		...
		public function send_popup(){
		
		$email =  $this->input->post('email');
		
		//check if email already exist
		$emails_where = $this->sm->getRecord('popup_registration',array('email' => $email));
		
		if(count($emails_where) > 0){//Email already exist
			$result = 2;
		}else{
			$inserted_id = $this->sm->create_popup_registration();
			if($inserted_id >0 ){
				$result = 1;
			}
		}
...
$this->email->send();
	...
	function request_callback(){
		...
		$data['account_number'] 		= $accountNumber = $this->input->post('accountNumber');
		$data['full_name'] 				= $name 		= $this->input->post('name');
		$data['phone_code'] 			= $phone_code 	= $this->input->post('phone_code');
		$data['phone'] 					= $phone 		= $this->input->post('phone');
		$data['email']  				= $email		= $this->input->post('email');
		$data['country_of_residence']	= $country 		= $this->input->post('country');
		$data['subject'] 				= $subject 		= $this->input->post('subject');
		$data['message']				= $message		= $this->input->post('message');
		$data['created_at']				= date('Y-m-d H:i:s');
		$data['website']				= $this->input->post('site_url');
		$data['lang_ref']				= $this->lg;
	
		$insert = $this->db->insert('tbl_request_callback',$data);
		header('Content-Type: application/json');
		
		$m = "<strong>We have received the following new request : <br><br></strong><table>";
		$m .= "<tr><td>Account Numer: </td><td>".$accountNumber."</td></tr>";
		$m .= "<tr><td>Name: </td><td>".$name."</td></tr>";
		$m .= "<tr><td>Phone: </td><td>".$phone."</td></tr>";
		$m .= "<tr><td>Email: </td><td>".$email."</td></tr>";
		$m .= "<tr><td>Country: </td><td>".$country."</td></tr>";
		$m .= "<tr><td>Subject: </td><td>".$subject."</td></tr>";
		$m .= "<tr><td>Message: </td><td>".$message."</td></tr>";
		$m .= "</table><br><br><hr>ICM Website<hr>";
		...
		$this->email->to('ibrahim.badran@icm.com');
		$this->email->subject('Request a Callback');
		$this->email->message($m);
		
		if ( $insert  || $this->email->send()) {
			//refresh captcha elements
			$rand1 = rand(1,10);
			$rand2 = rand(1,10);
			$result = $rand1+$rand2;
			
			$finalresult = array('success'=>1, 'rand1'=>$rand1, 'rand2'=>$rand2, 'result'=>$result);
			
		}
		else {
			//$finalresult = '<h4 style="color:red"><img src="assets/registration/images/landing/error.png" /> Try again please</h4>';
			$finalresult = array('success'=>0, 'rand1'=>10, 'rand2'=>10, 'result'=>20);
