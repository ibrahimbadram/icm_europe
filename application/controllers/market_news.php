...
class Market_news extends CI_Controller {
    ...
    public function index($url_title='') {
		...
		$data	= array();
		$check = $this->sm->getRecord('tbl_market_news',array('url_title'=>$url_title));
		if(  count($check) == 0 ){
		redirect(site_url('page_not_found'));
		}
		$data['check'] = $this->sm->getoneRecord('tbl_market_news',array('url_title'=>$url_title));
		...
		$this->template->content->view('news/market_news', $data);
