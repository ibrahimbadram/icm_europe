...
class Academy extends CI_Controller {
    ...
    public function index($title='') {
		...
		if ( $title == '' )
			$title = 'introduction';
				$filename = 'academy-'.$title;		if ( ! file_exists(APPPATH . 'views/'.$filename.'.php'))		{			show_404();		}		
		$newtitle = str_replace('-', ' ', $title);
        $this->template->title = $this->sm->getword('ICM',$this->lg).' - '.$this->sm->getword('Academy',$this->lg).' '.$this->sm->getword($newtitle,$this->lg);        $this->template->description = $this->sm->getword('ICM',$this->lg).' - '.$this->sm->getword('Academy',$this->lg).' '.$this->sm->getword($newtitle,$this->lg);		$this->template->trigger_meta('keyword',$this->sm->getword('Resources',$this->lg).' '.$this->sm->getword($newtitle,$this->lg));
       ...
       // $this->template->stylesheet->add('assets/css/common.css');
        ...
        $data = array();
		$breadcrumb['breadcrumb'] = array(0=>array('link'=>'home','title'=>$this->sm->getword('home',$this->lg)),1=>array('link'=>'academy','title'=>$this->sm->getword('Academy',$this->lg)),2=>array('link'=>'#','title'=>$this->sm->getword($newtitle,$this->lg)));
		$this->template->content->view('breadcrumb', $breadcrumb);
        $this->template->content->view('academy-'.$title, $data);
