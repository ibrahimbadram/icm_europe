...
class Promos extends CI_Controller {
    ...
    public function index($title='') {
		...
		if ( $title == '' )
		$title = 'bonuses';
		$filename = 'promos-'.$title;		if ( ! file_exists(APPPATH . 'views/'.$filename.'.php') )		{			show_404();		}
		$newtitle = str_replace('-', ' ', $title);
        $this->template->title = $this->sm->getword('ICM',$this->lg).' - '.$this->sm->getword('Promos',$this->lg).' '.$this->sm->getword($newtitle,$this->lg);        $this->template->description = $this->sm->getword('ICM',$this->lg).' - '.$this->sm->getword('Promos',$this->lg).' '.$this->sm->getword($newtitle,$this->lg);
		$this->template->trigger_meta('keyword',$this->sm->getword('Promos',$this->lg).' '.$newtitle);
       ...
       /* $this->template->stylesheet->add('assets/css/default.css');
        ...
        $data = array();
		$breadcrumb['breadcrumb'] = array(0=>array('link'=>'home','title'=>$this->sm->getword('home',$this->lg)),1=>array('link'=>'promos','title'=>$this->sm->getword('promos',$this->lg)),2=>array('link'=>'#','title'=>$this->sm->getword($newtitle,$this->lg)));
		$this->template->content->view('breadcrumb', $breadcrumb);				$data ['title'] = $this->sm->getword($newtitle,$this->lg);
        $this->template->content->view('promos-'.$title, $data);
